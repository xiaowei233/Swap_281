<template>
  <div id="add-blog">
    ...
    <vs-input
      ...
      v-model.lazy="userProfileEdit.username"
      :danger="usernameCheck"
      danger-text="Username is already taken"
      :success="usernameSafe"
      success-text="The username is valid"
      required
    />
    <vs-input
      class="vsinput2"
      label-placeholder="First Name"
      v-model.lazy="userProfileEdit.first_name"
      required
    />
    <vs-input
      class="vsinput2"
      label-placeholder="Last Name"
      v-model.lazy="userProfileEdit.last_name"
      required
    />
    <button
      class="btn btn-outline-secondary confirm-btn"
      v-on:click.prevent="post"
    >
      Confirm
    </button>
    <button class="btn btn-outline-secondary confirm-btn" @click="cancel">
      Cancel
    </button>
    <button class="btn btn-outline-secondary confirm-btn" @click="test">
      Test
    </button>
  </div>
</template>
...
export default {
  ...
  methods: {
    checkUsername: function() {
      ...
      UserDataService.compareUsername(this.userProfileEdit.username).then(
        res => {
    ...
    post: function() {
      this.errors = [];
      if (!this.userProfileEdit.username)
        this.errors.push("Username is requried");
      if (!this.userProfileEdit.first_name)
        this.errors.push("First name is requried");
      if (!this.userProfileEdit.last_name)
      ...
      UserDataService.edit(this.userProfileEdit).then(res => {
        ...
        this.$router.push(
          `/user/profile-edit-confirm/${this.userProfileEdit.id}`
        );
      });
    },
    cancel: function() {
