<template>
  <div id="add-blog">
    ...
    <form>
      ...
      />
      <button
        :class="{ username_danger: usernameCheck || usernameSafe }"
        class="btn btn-outline-secondary confirm-btn username-btn"
        v-on:click.prevent="checkUsername"
      ...
      <vs-input
        ...
        v-model.lazy="userRegister.password"
        required
      />
      <vs-input
        class="vsinput"
        ...
        v-model.lazy="userRegister.passwordConfirm"
        :danger="userRegister.password != userRegister.passwordConfirm"
    ...
    <vs-popup class="holamundo" title="Notification" :active.sync="popup">
      <p>This site currently only allows Lehigh users to register, please enter a valid Lehigh email address to register.</p>
    </vs-popup>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    checkEmailEnding: function() {
      ...
      return (
        this.userRegister.email.slice(
          this.userRegister.email.length - 11,
          this.userRegister.email.length
        ) != "@lehigh.edu"
      );
    },
    post: function() {
